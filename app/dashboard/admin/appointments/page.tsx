...
export default async function AdminAppointmentsPage({
  ...
  const [appointments, staff, services, locations] = await Promise.all([
    getAppointmentsForDay(organizationId, selectedDate),
            ...
            {staff.map((specialist) => {
              ...
              return (
                <div
                  ...
                  className="rounded-xl border border-zinc-200 bg-white p-4 shadow-2xs flex flex-col"
                >
                  <div className="flex items-center justify-between border-b border-zinc-100 pb-3 mb-3">
                    <div className="flex items-center gap-2">
                      <div className="flex h-8 w-8 items-center justify-center rounded-full bg-zinc-100 font-semibold text-xs text-zinc-700">
                  ...
                  <div className="space-y-3 flex-1">
                    {specialistAppointments.length === 0 ? (
                      <div className="flex flex-col items-center justify-center rounded-lg border border-dashed border-zinc-200 py-8 text-center">
                        <p className="text-xs text-zinc-400 italic">No appointments</p>
                      </div>
                    ) : (
                      specialistAppointments.map((appointment) => {
                        ...
                        return (
                          <article
                            key={appointment.id}
                            className="rounded-lg border border-zinc-200 bg-zinc-50/60 p-3 text-sm transition-shadow hover:shadow-xs"
                          ...
                          >
                            <div className="flex items-start justify-between gap-2">
                              ...
                              <span
                                className={`rounded-full px-2 py-0.5 text-2xs font-medium shrink-0 ${statusBadgeClass(
                                  appointment.status,
                                )}`}
                              >
                            ...
                            <div className="mt-2 text-xs text-zinc-600 space-y-0.5 border-t border-zinc-200/60 pt-2">
                              <p className="font-medium text-zinc-900">
                                ...
                                {contact.customerEmail ? (
                                  <span className="font-normal text-zinc-400 text-2xs ml-1">
                                    ({contact.customerEmail})
                                  </span>
                                ) : null}
                              </p>
                              ...
                              <p className="font-medium text-zinc-900 pt-0.5">
