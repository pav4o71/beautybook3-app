  ...
  return (
    <header className="border-b border-emerald-100/80 bg-white/90 backdrop-blur-sm">
      <div className={`${pageShellClass} flex items-center justify-between gap-3 py-3`}>
        <Link href="/" className={`${brandLinkClass} text-emerald-950`}>
          BeautyBook
        </Link>
        <nav className="flex items-center gap-2 text-sm sm:gap-3">
          {session?.user ? (
            ...
            <Link href="/login" className={`${secondaryButtonClass} border-emerald-200 text-emerald-900 hover:bg-emerald-50`}>
              Sign in
            </Link>
          )}
        </nav>
